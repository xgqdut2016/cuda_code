    ...
    for (int ph = 0; ph < width; ph++)
    {
        for (int index_k = 0; index_k < BK; index_k++)
        {
            for (int index_q = 0; index_q < TM; index_q++)
            {
                ...
                {
                    SA[threadIdx.x * TM + index_q][index_k] = dA[(indA + index_q) * K + index_k + ph * BK];
                }
                else
                {
                    SA[threadIdx.x * TM + index_q][index_k] = 0.0f;
                }
            }
            ...
            for (int index_v = 0; index_v < TN; index_v++)
            {
                ...
                {
                    SB[index_k][threadIdx.y * TN + index_v] = dB[(index_k + ph * BK) * N + indB + index_v];
                }
                else
                {
                    SB[index_k][threadIdx.y * TN + index_v] = 0.0f;
                }
            }
        }
        __syncthreads();
        for (int index_q = 0; index_q < TM; index_q++)
        {
            for (int index_v = 0; index_v < TN; index_v++)
            {
                for (int index_k = 0; index_k < BK; index_k++)
                {
                    tmp[index_q][index_v] += SA[threadIdx.x * TM + index_q][index_k] * SB[index_k][threadIdx.y * TN + index_v];
                }
            }
        }
        __syncthreads();
