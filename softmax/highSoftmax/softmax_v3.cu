...
void _softmax_kernel(float *input, int size, int *inputShape, int axis, int nDims, int stride){
    ...
    if(threadIdx.x < dimsize) share_input[threadIdx.x] = input[tid + threadIdx.x * stride];
    for(int ph = 0; threadIdx.x + ph*blockDim.x < dimsize; ph++){
        MD md_input;
        if(ph == 0){
            md_input.max_tmp = share_input[threadIdx.x];
        }
        else{
            md_input.max_tmp = input[tid + (threadIdx.x + ph*blockDim.x)*stride];
        }
    ...
    for (int ph = 0; threadIdx.x + ph*blockDim.x < dimsize; ph++) {
        if(ph == 0){
            input[tid + (threadIdx.x) * stride] =
            __expf(share_input[threadIdx.x] - max_total) * sum_inverse_total;
        }
        else{
            input[tid + (threadIdx.x + ph*blockDim.x) * stride] =
            __expf(input[tid + (threadIdx.x + ph*blockDim.x) * stride] - max_total) * sum_inverse_total;
        }
...
void softmax(float *cpu_input, int size, int *cpu_inputShape, int axis, int nDims){
    ...
    if (dimsize > 1023) {
        int BLOCK_DIM = 1024;
        int share_mem = (2 + BLOCK_DIM)*sizeof(float);
