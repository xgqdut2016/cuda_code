                                   ...
                                   int dimsize, int stride)
{
    ...
    for (int ph = 0; threadIdx.x + ph * warpSize < dimsize; ph++)
    {
        if (otherIdx < otherSize)
        {
            max_data = max_function(
                max_data, input[tid + (threadIdx.x + ph * warpSize) * stride]);
        }
    }
    if (dimsize > warpSize / 2)
    {
        max_data = WarpAllReduce<MaxOp, float, warpSize>(max_data);
    }
    else if (dimsize > warpSize / 4)
    {
        max_data = WarpAllReduce<MaxOp, float, warpSize / 2>(max_data);
    }
    else
    {
        max_data = WarpAllReduce<MaxOp, float, warpSize / 4>(max_data);
    }
    ...
    for (int ph = 0; threadIdx.x + ph * warpSize < dimsize; ph++)
    {
        ...
        {
            sum_data +=
                __expf(input[tid + (threadIdx.x + ph * warpSize) * stride] -
    ...
    {
        sum_data = WarpAllReduce<SumOp, float, warpSize / 2>(sum_data);
    }
    else
    {
        sum_data = WarpAllReduce<SumOp, float, warpSize / 4>(sum_data);
    }
