...
{ // if set axis = 1, inputShape=[I,J,K,S]
  // tid = i(JKS) + j(KS) + k(S) + s
    ...
    int extra = dimsize % BLOCK_DIM;
    int step = (dimsize - extra) / BLOCK_DIM;
    ...
    if (threadIdx.x < extra)
    {
        ...
        {
            dms_input.max_tmp = input[tid + ind * stride];
                                   ...
                                   int dimsize, int stride)
{
    ...
    int extra = dimsize % BLOCK_DIM_x;
    int step = (dimsize - extra) / BLOCK_DIM_x;
    if (otherIdx < otherSize)
    {
        ...
        if (threadIdx.x < extra)
        {
            for (int ind = threadIdx.x * (step + 1);
                 ind < (threadIdx.x + 1) * (step + 1); ind++)
            {
                max_data = max(max_data, input[tid + ind * stride]);
            }
        }
        else
        {
        ...
        if (threadIdx.x < extra)
        {
            ...
            {
                sum_data +=
                    __expf(input[tid + ind * stride] - max_total[threadIdx.y]);
            }
        }
        else
        {
