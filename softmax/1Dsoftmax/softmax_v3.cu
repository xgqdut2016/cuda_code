...
__global__
void softmax_kernel(float *input, int size, float *result, int num_blocks, int reduce){
...
void cpu_softmax(float *cpu_input, int size, int reduce){
    ...
    softmax_kernel<<<grid_dim, block_dim, share_size>>>(input, size, result, num_blocks, reduce);
